...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    /*
    Cost Function for cte,epsi,
    we add cost pow(2) to cte and epsi 
    though ref v was given in the quiz.no such reference velocity has been provided here and wont be including that one.
    */
  //Adding cost of cte and epsi
    for(int i=0;i<N;i++)
    {
      fg[0]+=3000*CppAD::pow(vars[cte_start+i],2);
      fg[0]+=3000*CppAD::pow(vars[epsi_start+i],2);
      fg[0]+=10*CppAD::pow(ref_v-vars[v_start+i],2);
    }
  //adding cost of delta_t,accelaratiom
    ...
    //adding cost of change of delta_t,and accelaration. Delta_t is multiplited by a value>1 for smoothing the change in delta_t
    for(int i=0;i<N-2;i++)
    {
    ...
    /*
    Adding contraints to the model parameters,ie x,y,velocity,psi,epsi,cte;
    */
   ...
   for(int i=1;i<N;i++)
   {
     ...
     fg[psi_start+i+1] = psi1 - (psi0+(v0/Lf)*delta0*dt);

     AD<double> cte1 = vars[cte_start+i];
     AD<double> epsi0= vars[epsi_start+i-1];
     AD<double> f0 = CppAD::pow(x0,3)*coeffs[3]+CppAD::pow(x0,2)*coeffs[2]+x0*coeffs[1]+coeffs[0];
     fg[cte_start+i+1] = cte1 -((f0-y0)+(v0*CppAD::sin(epsi0)*dt));

     AD<double> temp_result = (3*coeffs[3]*CppAD::pow(x0,2))+(2*coeffs[2]*x0)+(coeffs[1]);
     AD<double> depsi0 = CppAD::atan(temp_result);
     AD<double> epsi1 = vars[epsi_start+i];
     fg[1 + epsi_start + i] =epsi1 - ((psi0 - depsi0) + v0 * delta0 / Lf * dt);
